...
const char* WIFI_PASSWORD = "swaggysarang";
const char* BACKEND_IP    = "172.20.10.2";
...
void loop() {
  ...
  if (currentMode != MODE_CART) {
    ...
    else if (nav == 'P') setMode(MODE_PAYMENT);
  ...
  if (currentMode == MODE_PAYMENT) {
    ...
    if (method != '\0') {
      String paymentMethod = method == 'C' ? "card" : method == 'T' ? "tap" : "cash";
      display_showPayment(runningTotal, "Processing " + paymentMethod + "...");
      PaymentResult payment = apiClient_checkout(paymentMethod);
      lastPaymentMessage = payment.message;
      if (payment.success) {
        runningTotal = 0;
        cartItems.clear();
        lastKnownTotal = 0;
