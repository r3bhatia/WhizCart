...
void display_showTotal(float total, float measuredWeightG) {
  ...
  gfx->setCursor(24, CONTENT_Y + 316);
...
void display_showPayment(float total, String status) {
  ...
  int gap = 18;
  int buttonW = (screenW() - 48 - gap * 2) / 3;
  const char* labels[3] = { "CARD", "TAP", "CASH" };

  for (int i = 0; i < 3; i++) {
    int x = 24 + i * (buttonW + gap);
    gfx->fillRoundRect(x, buttonY, buttonW, 88, 8, PANEL_COLOR);
    gfx->drawRoundRect(x, buttonY, buttonW, 88, 8, BORDER_COLOR);
    gfx->setTextSize(3);
    gfx->setTextColor(TEXT_MAIN, PANEL_COLOR);
    gfx->setCursor(x + (buttonW / 2) - 38, buttonY + 32);
    gfx->print(labels[i]);
  }
...
char display_getPaymentTap() {
  ...
  for (int i = 0; i < 3; i++) {
    int x = 24 + i * (buttonW + gap);
    if (p.x >= x && p.x <= x + buttonW) {
      waitForTouchRelease();
      if (i == 0) {
        Serial.println("[Touch] Pay card");
        return 'C';
      }
      if (i == 1) {
        Serial.println("[Touch] Pay tap");
        return 'T';
      }
      Serial.println("[Touch] Pay cash");
      return 'A';
    }
  }
...
char display_getNavTap(Mode currentMode) {
  ...
  if (currentMode != MODE_TOTAL && p.x <= 170 && p.y >= 400) {
    waitForTouchRelease();
    Serial.println("[Touch] Back lower-left");
    return 'B';
  }

  // Some 7-inch GT911 boards report the drawn bottom nav strip near the top
  // edge. Accept that mirrored top hitbox so the visible Cart/Recs buttons work.
  if (currentMode == MODE_TOTAL && p.y <= 80) {
    waitForTouchRelease();
    if (p.x < screenW() / 2) {
